...
`<mark>`、`<sub>`、`<sup>` HTML 標籤也可作為輸入使用，儲存文件時會被正規化
（normalize）為上方的 `==`／`~`／`^` 簡寫語法。
...
不同詞彙之間需要空一行——若沒有空行，第二個詞彙會被視為前一個定義的延續
（標準 Markdown 段落規則），而不是新的詞彙。直接使用 `<dl>`／`<dt>`／`<dd>`
的原始 HTML 區塊一樣可行。
...
輸入或匯入時，短代碼會自動轉換成對應的 Unicode emoji 字元（😄）。這是單
向轉換：文件會以 emoji 字元本身儲存，而不是 `:smile:` 短代碼。
